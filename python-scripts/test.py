...
program_path = os.path.join(program_path, "..")
width = 0.35
    ...
    for file in files:
        if file.endswith("_confusion_matrice.csv"):
            ...
            with open(os.path.join(program_path, "PROCESS", "graph", file)) as csv_file:
                ...
                # plt.show()
                plt.savefig(os.path.join(program_path, 'PROCESS', 'graph', file + '.jpg'))

            csv_data = []
            with open(os.path.join(program_path, 'PROCESS', 'graph', "distance_success_rate.csv")) as csv_file:
                csv_reader = csv.reader(csv_file, delimiter=',')
                for row in csv_reader:
                    csv_data.append(row)
